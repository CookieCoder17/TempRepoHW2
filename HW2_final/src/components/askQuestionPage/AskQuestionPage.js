...
function AskQuestionPage({ model, updatePage }) {
  ...
  const validateQuestion = () => {
    ...
    if (question.content === '') {
      ...
      if(foundError){
        newStateHasError = { ...newStateHasError, content: '*Constraints violated. The target of a hyperlink, that is, the stuff within () cannot be empty and must begin with “https://” or “http://”.'};
      }
    }
  ...
  function tagMaintenance(tagNames) {
    ...
    for (const tagName of tagNames) {
      let found = false;
      let tags = model.getAllTags();

      for (const element of tags) {
        if (tagName === element.name) {
          tagIds.push(element.tid);
          found = true;
          break;
        }
      }
